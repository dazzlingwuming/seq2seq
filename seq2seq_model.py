...
class Seq2Seq(nn.Module):
    ...
    def forward(self, encode_x, decode_x, decode_y):
        ...
        decode_x = self.embeding_layer(embeding_x)#[N,T2]->[N,T2,E]
        #编码器
        encode_x, (h_e, c_e) = self.encoder_layer(encode_x)#这里的encode_x是没有什么作用的,h_e[batch, num_layers*num_directions,hidden_size] c_e[batch, num_layers*num_directions,hidden_size]
        #解码器
    ...
    def predict(self , x , max_length :int = 30):
        ...
        input_x = torch.tensor([0]*x.size(0), dtype=torch.long).unsqueeze(1).to(x.device)#假设0是SOS,[N,1],decode_x[0] = SOS
        #这里需要一个循环
        outputs = []
...
if __name__ == '__main__':
    ...
    decode_x = torch.tensor([0, 2, 3, 4, 5] ,dtype = torch.long)#["<eos>", "B", "C", "D", "E"]
